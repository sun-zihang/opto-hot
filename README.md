...
- [x] 微信公众号覆盖更全：多查询 + 会话 Cookie + 反爬检测 + 公众号/时间/摘要字段（90 天新鲜度过滤）
- [x] 政策 / 招投标结构化库（`data/biz.json`：项目号 / 预算 / 截止时间，详情页解析）
- [x] 真嵌入聚类（CI 内置 bge-small-zh + HF 缓存；TF-IDF 事件聚类 + 神经近重复合并）与主题可解释性（`top_terms`）
- [ ] 微信验证码彻底绕过（受平台策略限制，采用多查询 + 降级处理缓解）
